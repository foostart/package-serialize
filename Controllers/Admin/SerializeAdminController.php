<?php namespace Foostart\Serialize\Controllers\Admin;

/*
|-----------------------------------------------------------------------
| SerializeAdminController
|-----------------------------------------------------------------------
| @author: Kang
...
class SerializeAdminController extends FooController {
    ...
    public function __construct(Request $request) {
        ...
        //models
        $this->obj_item = new Serialize(array('perPage' => 10));
        $this->obj_category = new Category();
        $this->obj_slideshow = new Slideshow();

        //validators
        $this->obj_validator = new SerializeValidator();

        //set language files
        $this->plang_admin = 'serialize-admin';
        $this->plang_front = 'serialize-front';
    ...
    public function config(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/package-serialize.php'));
        $package_path = realpath(base_path('vendor/foostart/package-serialize'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            file_put_contents($config_bakup.'/package-serialize-'.date('YmdHis',time()).'.php', $content);
    ...
    public function lang(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $langs = config('package-serialize.langs');
        $lang_paths = [];
        $package_path = realpath(base_path('vendor/foostart/package-serialize'));

        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/'.$key.'/serialize-admin.php'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                //format file name serialize-admin-YmdHis.php
                file_put_contents($lang_bakup.'/'.$key.'/serialize-admin-'.date('YmdHis',time()).'.php', $content);
            }
